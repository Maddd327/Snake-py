...
def MyCalc(source, side):
    sObj = Frame(source, borderwidth=4, bd=4, bg='white')
...
class app(Frame):
    def __init__(self):
        ...
        self.master.title('Kalkulator Pertamaku')
        
        display = StringVar()
        Entry(self, relief=FLAT, textvariable=display, bd=30, bg='silver').pack(side=TOP, expand=YES, fill=BOTH)
